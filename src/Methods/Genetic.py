...
class Genetic:
    def __init__(self, agentsNum, modelPath=None):
    ...
    def predict(self, statesArr: (List[List[float]], List[bool])) -> List[List[float]]:
        return [model.predict(state)[0] if active else None for model, state, active in zip(self.usedModels, statesArr[0], statesArr[1])]
    ...
    def train(self, statesArr: (List[List[float]], List[bool]), rewardsArr: List[float], cumRewards: List[float], done: bool):
        if done:
